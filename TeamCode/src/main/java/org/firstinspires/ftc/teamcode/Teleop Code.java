...
public class FTC_RoverTeleOp extends LinearOpMode {
    ...
    public void runOpMode() {
        ...
        while (opModeIsActive()) {
            ...
            double rotate = robot.scalePower(Range.clip(gamepad1.left_stick_x, -1.0, 1.0));
            //double rotate2 = robot.scalePower(Range.clip(gamepad2.left_stick_x, -0.5, 0.5));
            double strafe = robot.scalePower(Range.clip(gamepad1.right_stick_x, -1.0, 1.0));
            //double strafe2 = robot.scalePower(Range.clip(gamepad2.right_stick_x, -0.5, 0.5));
            double drive = robot.scalePower(Range.clip(   gamepad1.right_stick_y, -1.0, 1.0));
            //double drive2 = robot.scalePower(Range.clip(   gamepad2.right_stick_y, -0.5, 0.5));
            ...
            final double v1 = Range.clip(drive + strafe + rotate,-1.0/mode,1.0/mode); // Left front drive
            final double v2 = Range.clip(drive - strafe - rotate  ,-1.0/mode,1.0/mode); // Right front drive
            final double v3 = Range.clip(drive + strafe - rotate ,-1.0/mode,1.0/mode); // Left rear drive
            final double v4 = Range.clip(drive - strafe + rotate,-1.0/mode,1.0/mode); // Right rear drive
